...
*** Jobs UI panel will no longer show job number 123.
...
*** You will see all employees except for the deleted employee of index 1 in employees UI panel.
. `undo` (reverses the `delete 1` command) +
*** You will see the deleted employee present in the employees UI panel again.

** Example 2: +
. `closej 1` +
*** You will see that job number 1 is removed from the jobs UI panel.
...
** Example 2: +
. `closej 1` +
*** You will see that the job number 1 is removed from the jobs UI panel.
. `deletee 1` +
*** You will see that the first employee is removed from the employees UI panel.
. `undo` (reverses the `deletee 1` command) +
*** You will see that the deleted employee is added back into the employees UI panel.
...
=== Email assigned employees belonging to a job : `email JOB_NUMBER`

You can use this command to send a notice email to all assigned employees to job of JOB_NUMBER. +
